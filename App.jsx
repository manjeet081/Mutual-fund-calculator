...
import {
  ...
  ResponsiveContainer, Area, AreaChart, Line, ReferenceLine
} from "recharts";
...
export default function App() {
  ...
  const updateLumpsum = (i, field, val) => {
    ...
    updated[i] = { ...updated[i], [field]: field === "label" ? val : Number(val) };
  ...
  const chartData = useMemo(() => {
    ...
    for (let y = 1; y <= Number(years); y++) {
      ...
      for (let m = 0; m < 12; m++) {
        ...
        if (sipActive) { corpus += currentSip; sipThisYear += currentSip; }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Space Mono', monospace", color: C.text, padding: "24px 16px" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "340px 1fr", gap: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20 }}>
            ...
            {extraLumpsums.map((e, i) => (
              <div key={i} style={{ marginBottom: 14, background: C.surface, borderRadius: 10, padding: 10, border: `1px solid ${C.border}` }}>
                ...
                <div>
                  <div style={{ fontSize: 10, color: C.muted, marginBottom: 5, letterSpacing: "0.1em" }}>LABEL (optional)</div>
                  <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20 }}>
            ...
            {activeTab === "breakdown" && (
              <>
                <div style={{ fontSize: 11, color: C.muted, marginBottom: 14 }}>Wealth gained vs corpus — compounding power</div>
                <ResponsiveContainer width="100%" height={290}>
                  <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                    <defs>
      ...
      <div style={{ maxWidth: 1100, margin: "14px auto 0", fontSize: 10, color: C.muted, textAlign: "center", lineHeight: 1.6 }}>
        * Illustrative only. Returns not guaranteed. Consult a SEBI-registered investment advisor.
      </div>
    </div>
  );
}
